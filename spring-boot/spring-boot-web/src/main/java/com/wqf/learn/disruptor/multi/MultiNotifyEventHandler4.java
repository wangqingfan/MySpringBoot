...
@Builder
@NoArgsConstructor
public class MultiNotifyEventHandler4 implements EventHandler<NotifyEvent>, WorkHandler<NotifyEvent> {

    @Override
    public void onEvent(NotifyEvent notifyEvent) throws Exception {
        System.out.println("handler4--接收到消息");
        this.onEvent(notifyEvent);
    }

    @Override
    public void onEvent(NotifyEvent notifyEvent, long l, boolean b) throws Exception {
        System.out.println("handler4--"+notifyEvent.toString() + ">>>" + Thread.currentThread().toString());
        Thread.sleep(1*1000);
        notifyEvent.setMessgae("m4");
    }
}
