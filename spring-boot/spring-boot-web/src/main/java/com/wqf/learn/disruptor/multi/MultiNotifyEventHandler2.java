...
@Builder
@NoArgsConstructor
public class MultiNotifyEventHandler2 implements EventHandler<NotifyEvent>, WorkHandler<NotifyEvent> {

    @Override
    public void onEvent(NotifyEvent notifyEvent) throws Exception {
        System.out.println("handler2--接收到消息");
        this.onEvent(notifyEvent);
    }

    @Override
    public void onEvent(NotifyEvent notifyEvent, long l, boolean b) throws Exception {
        System.out.println("handler2--"+notifyEvent.toString() + ">>>" + Thread.currentThread().toString());
        Thread.sleep(1*1000);
        notifyEvent.setMessgae("m2");
    }
}
